...
const isUserRoleAdminOrOrganizer = (req, res, next) => {
    const userData = req.auth;
    if (userData.role === userRole.SYSTEM_ADMIN || userData.role === userRole.ORGANIZER) {
        next();
    } else {
        return res.status(API_STATUS_CODE.NOT_ACCEPTABLE).send({
            status: "failed",
...
const isUserRoleAdminOrVisitor = (req, res, next) => {
    const userData = req.auth;
    if (userData.role === userRole.SYSTEM_ADMIN || userData.role === userRole.VISITOR) {
        next();
    } else {
        return res.status(API_STATUS_CODE.NOT_ACCEPTABLE).send({
            status: "failed",
...
const isUserRoleAdminOrExhibitorAdminOrExhibitor = (req, res, next) => {
    const userData = req.auth;
    if (userData.role === userRole.SYSTEM_ADMIN || userData.role === userRole.EXHIBITOR_ADMIN || userData.role === userRole.EXHIBITOR) {
        next();
    } else {
        return res.status(API_STATUS_CODE.NOT_ACCEPTABLE).send({
            status: "failed",
...
const isUserRoleExhibitorAdminOrExhibitorOrVisitor = (req, res, next) => {
    const userData = req.auth;
    if (userData.role === userRole.EXHIBITOR_ADMIN || userData.role === userRole.EXHIBITOR || userData.role === userRole.VISITOR) {
        next();
    } else {
        return res.status(API_STATUS_CODE.NOT_ACCEPTABLE).send({
            status: "failed",
