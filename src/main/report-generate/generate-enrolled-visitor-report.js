...
const generateBodyData = async (lgKey, bodyData) => {
    ...
    if (lgKey === 'ja') {
        tableHeader = ["名前", "メール", "連絡先", "会社", "役職", "出席状況", "作成日時", "更新日時"];
 ...
 */
const generateEnrolledVisitorReport = async (bodyData) => {
    ...
    try {
        ...
        if (lgKey === 'ja') {
            document.setFont("NotoSansJP", "bold")
                .setFontSize(25)
                .text(`登録来場者レポートを作成しました`, leftMargin, 65)
            ...
            document.setFont("NotoSansJP", "bold")
                .setFontSize(25)
                .text(`Generated Enrolled Visitor Report`, leftMargin, 65)
        ...
        const filePath = path.join(fileSavePath, "enrolled-visitor-report.pdf");
