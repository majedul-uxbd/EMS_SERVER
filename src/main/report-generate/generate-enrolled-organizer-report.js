...
const generateBodyData = async (lgKey, bodyData) => {
    ...
    if (lgKey === 'ja') {
        tableHeader = ["名前", "メール", "連絡先", "役職", "出席状況", "作成日時", "更新日時"];
        ...
        tableHeader = ["Name", "Email", "Contact", "Position", "Attendance", "Created Time", "Updated Time"];
        for (let i = 0; i < bodyData.length; i++) {
            const record = bodyData[i];
            const fullName = `${record.f_name} ${record.l_name}`.trim();
            const row = [
...
/**
 * @description This function is used to generate enrolled Organizer report 
 */
const generateEnrolledOrganizerReport = async (bodyData) => {
    ...
    try {
        ...
        if (lgKey === 'ja') {
            document.setFont("NotoSansJP", "bold")
                .setFontSize(25)
                .text(`登録ボランティアレポートを作成しました`, leftMargin, 65)
            ...
            document.setFont("NotoSansJP", "bold")
                .setFontSize(25)
                .text(`Generated Enrolled Volunteers Report`, leftMargin, 65)
        ...
        const filePath = path.join(fileSavePath, "enrolled-volunteers-report.pdf");
...
module.exports = {
    generateEnrolledOrganizerReport
}